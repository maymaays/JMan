...
import javax.swing.*;
import jman.piece.Block;
import jman.piece.JMan;
import jman.piece.Pillar;
import jman.piece.Walker;
			...
			for (int i = 0; i < m.width; i = i + 1) {
				for (int j = 0; j < m.height; j = j + 1) {
					...
					if (m.grid[i][j] != null && m.grid[i][j].getType() == Piece.BLOCK) {
						...
						// triangle.
						g.setColor(m.grid[i][j].getColor());
						g.fillPolygon(new int[] { h + 1, h1 - 1, h + m.tileWidth / 2 }, new int[] { v1 - 2, v1 - 2, v },
								3);
					}
				}
			} // end of for loops
